        ...
        for i in range(n):
            for j in range(n):
                self.__canvas.create_rectangle(
                    ...
                    fill='black' if (i & 1) ^ (j & 1) else 'white', outline='')
        ...
        def display():
            ...
            for i, pos in enumerate(solutions[self.__cur_sol]):
                queens.append(self.__canvas.create_image(
                    (i * pixel_per_block, pos * pixel_per_block),
                    image=self.__queen_image, anchor=tk.NW))
            self.__cur_sol += 1
            if self.__cur_sol >= len(solutions):
                button.config(state=tk.DISABLED)

        display()

        button = tk.Button(text="Next solution", command=display)
        self.__canvas.create_window(10, 10, window=button, anchor=tk.NW)
